...
export function DirectContactCard({ listingId, region, sellerLabel, status }: DirectContactCardProps) {
  ...
  const isWanted = status === "구매요청";
  const contactOwner = isWanted ? "구매 희망자" : "판매자";
  ...
  const copyDemoContact = async () => {
    ...
    try {
      ...
      setNotice(`공개 데모 연락처를 복사했습니다. 실제 서비스에서는 ${contactOwner}가 등록한 인증 번호가 표시됩니다.`);
    } catch {
  ...
  return (
    <section id="direct-contact" className="mt-4 rounded-xl border border-border bg-white p-5 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-semibold text-text-muted">{isWanted ? "구매 희망자와 직접 연결" : "판매자와 직접 거래"}</p>
          <h2 className="mt-1 text-lg font-bold text-text-primary">{sellerLabel}</h2>
          <p className="mt-1 text-sm text-text-secondary">{region} · {contactOwner}에게 전화·문자 직접 연락</p>
        </div>
        <span className="rounded-full bg-brand/10 px-3 py-1.5 text-xs font-bold text-brand">휴대폰 확인 {contactOwner} 예시</span>
      </div>
      ...
      {status === "판매중" || status === "구매요청" ? (
        verified ? (
          <div className="mt-4">
            {!revealed ? (
              ...
              <div className="rounded-lg border border-brand/20 bg-brand/5 p-4">
                ...
                <p className="mt-1 text-xs text-text-muted">실제 서버 연결 후에는 {contactOwner}가 인증하고 공개한 번호가 표시됩니다.</p>
          ...
          <div className="mt-4">
            ...
            <p className="mt-2 text-center text-xs text-text-muted">{contactOwner} 연락처 보호와 거래 책임 확인을 위해 휴대폰 인증이 필요합니다.</p>
          </div>
        )
      ) : (
      ...
      {(status === "판매중" || status === "구매요청") && (
        <div className="fixed inset-x-0 bottom-0 z-[60] border-t border-border bg-white p-2 shadow-[0_-6px_20px_rgba(17,24,39,0.1)] lg:hidden">
          {verified ? (
            <button type="button" onClick={openContact} className="w-full rounded-lg bg-accent px-4 py-3 text-sm font-bold text-white">{contactOwner} 연락처 보기</button>
          ) : (
            <Link href={`/login?next=/listings/${listingId}`} className="block w-full rounded-lg bg-accent px-4 py-3 text-center text-sm font-bold text-white">인증 후 {contactOwner}에게 직접 연락</Link>
          )}
        </div>
      )}
    </section>
