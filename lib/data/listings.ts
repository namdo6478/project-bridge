import type { Listing } from "@/lib/types/listing";

export const listings: Listing[] = [
  {
    id: "1",
    sellerId: "seller-gb-farm",
    title: "대형 사각압축포장기 2020년식",
    category: "조사료 장비",
    subcategory: "사각압축포장기",
    region: "경북",
    year: 2020,
    price: 85_000_000,
    priceNegotiable: false,
    status: "판매중",
    condition: "중고",
    manufacturer: "KR Pack",
    model: "KP-800",
    usageHours: 1240,
    tradeOptions: ["시운전 가능", "운송 협의 가능", "정비 이력 있음"],
    description:
      "2019년 구입 후 연 2회 시즌 사용한 대형 사각압축포장기입니다. 실린더 및 체인 상태 양호하며, 현장 시운전 가능합니다. 운송비는 별도 협의합니다.",
    createdAt: "2026-07-01",
    confirmedAt: "2026-07-16",
  },
  {
    id: "2",
    sellerId: "seller-cn-company",
    title: "5톤 TMR 사료배합기",
    category: "사료 장비",
    subcategory: "TMR 배합기",
    region: "충남",
    year: 2022,
    price: null,
    priceNegotiable: true,
    status: "판매중",
    condition: "중고",
    manufacturer: "FeedMix Pro",
    model: "FM-5000",
    usageHours: 860,
    tradeOptions: ["가격 협의 가능", "시운전 가능", "운송 협의 가능"],
    description:
      "5톤급 TMR 사료배합기로, 사료 균일 혼합 성능이 우수합니다. 스크류 및 교반 날 상태 양호하며, 전기 제어판 교체 완료했습니다. 가격은 현장 확인 후 협의 가능합니다.",
    createdAt: "2026-07-03",
    confirmedAt: "2026-07-17",
  },
  {
    id: "3",
    sellerId: "seller-jb-farm",
    title: "사료 이송 컨베이어 30m",
    category: "사료 장비",
    subcategory: "컨베이어",
    region: "전북",
    year: 2021,
    price: 32_000_000,
    priceNegotiable: false,
    status: "판매중",
    condition: "중고",
    manufacturer: "ConveyTech",
    model: "BC-30",
    tradeOptions: ["운송 협의 가능", "설치 협의 가능", "정비 이력 있음"],
    description:
      "축사 내 사료 이송용 벨트 컨베이어 30m 세트입니다. 모터 및 베어링 최근 교체했으며, 분해·운송·설치 지원 가능합니다.",
    createdAt: "2026-07-05",
    confirmedAt: "2026-07-15",
  },
  {
    id: "4",
    sellerId: "seller-gb-farm",
    title: "로더용 베일집게 1.8m",
    category: "조사료 장비",
    subcategory: "베일집게",
    region: "강원",
    year: 2019,
    price: 18_000_000,
    priceNegotiable: false,
    status: "예약중",
    condition: "중고",
    manufacturer: "AgriGrip",
    model: "BG-180",
    usageHours: 530,
    tradeOptions: ["시운전 가능"],
    description:
      "로더 부착형 베일집게로 건초·짚 베일 적재에 적합합니다. 유압 실린더 및 핀 상태 양호하며, 현재 예약 진행 중입니다.",
    createdAt: "2026-07-06",
    confirmedAt: "2026-07-16",
  },
  {
    id: "5",
    sellerId: "seller-gg-equipment",
    title: "카멜 절단기 중고",
    category: "조사료 장비",
    subcategory: "절단기",
    region: "경기",
    year: 2018,
    price: 9_500_000,
    priceNegotiable: false,
    status: "판매완료",
    condition: "중고",
    manufacturer: "CutMaster",
    model: "CM-450",
    usageHours: 1780,
    tradeOptions: ["정비 이력 있음"],
    description:
      "목초·사료 절단용 카멜 절단기입니다. 날 교체 이력 있으며 작업 효율이 우수합니다. 판매가 완료된 매물입니다.",
    createdAt: "2026-06-20",
    confirmedAt: "2026-07-12",
  },
  {
    id: "6",
    sellerId: "seller-jeju-shop",
    title: "자동 사료공급기 500L",
    category: "신품관",
    subcategory: "신품 장비",
    region: "제주",
    year: 2023,
    price: 4_500_000,
    priceNegotiable: false,
    status: "판매중",
    condition: "신품",
    manufacturer: "AutoFeed",
    model: "AF-500",
    tradeOptions: ["운송 협의 가능", "설치 협의 가능", "A/S 1년"],
    description:
      "500L 용량 자동 사료공급기 신품 재고분입니다. 타이머 제어 및 잔량 센서 포함, 설치 매뉴얼 및 A/S 1년 제공합니다.",
    createdAt: "2026-07-08",
    confirmedAt: "2026-07-17",
  },
  {
    id: "7",
    sellerId: "seller-cn-company",
    title: "분뇨 처리 스크래퍼",
    category: "축사시설·설비",
    subcategory: "분뇨처리기",
    region: "충북",
    year: 2017,
    price: null,
    priceNegotiable: true,
    status: "판매중",
    condition: "중고",
    manufacturer: "CleanFarm",
    model: "CS-200",
    usageHours: 2100,
    tradeOptions: ["가격 협의 가능", "시운전 가능"],
    description:
      "축사 바닥 분뇨 처리용 스크래퍼 장비입니다. 레일 및 구동부 정상 작동하며, 현장 실물 확인 후 가격 협의합니다.",
    createdAt: "2026-07-09",
    confirmedAt: "2026-07-14",
  },
  {
    id: "8",
    sellerId: "seller-jb-farm",
    title: "중고 사각압축기 2018년",
    category: "조사료 장비",
    subcategory: "사각압축포장기",
    region: "전남",
    year: 2018,
    price: 62_000_000,
    priceNegotiable: false,
    status: "판매중",
    condition: "중고",
    manufacturer: "KR Pack",
    model: "KP-700",
    usageHours: 1680,
    tradeOptions: ["시운전 가능", "정비 이력 있음"],
    description:
      "2018년식 중형 사각압축기입니다. 정기 점검 이력 있으며, 포장 밀도 조절 기능 정상 작동합니다. 시범 작업 가능합니다.",
    createdAt: "2026-07-10",
    confirmedAt: "2026-07-16",
  },
  {
    id: "9",
    sellerId: "seller-gg-equipment",
    title: "소형 사료배합기 1톤",
    category: "사료 장비",
    subcategory: "TMR 배합기",
    region: "경남",
    year: 2020,
    price: 28_000_000,
    priceNegotiable: false,
    status: "판매중",
    condition: "중고",
    manufacturer: "FeedMix Pro",
    model: "FM-1000",
    usageHours: 920,
    tradeOptions: ["시운전 가능", "운송 협의 가능"],
    description:
      "소규모 축사용 1톤 사료배합기입니다. 전기 380V, 교반 시간 설정 가능하며, 소형 트랙터와 연동 작업에 적합합니다.",
    createdAt: "2026-07-11",
    confirmedAt: "2026-07-15",
  },
  {
    id: "10",
    sellerId: "seller-cn-company",
    title: "컨베이어 구동모터·체인 세트",
    category: "부품·소모품",
    subcategory: "구동부품",
    region: "충남",
    year: 2022,
    price: null,
    priceNegotiable: true,
    status: "판매중",
    condition: "중고",
    manufacturer: "ConveyTech",
    model: "CT-DRIVE",
    tradeOptions: ["가격 협의 가능", "설치 협의 가능"],
    description:
      "사료·베일 컨베이어용 구동모터와 체인 세트입니다. 장비 규격에 맞춰 구성 협의 가능하며, 설치 조건에 따라 견적을 조정합니다.",
    createdAt: "2026-07-12",
    confirmedAt: "2026-07-17",
  },
  {
    id: "11",
    sellerId: "buyer-gb-farm",
    title: "중고 베일집게 삽니다",
    category: "삽니다",
    subcategory: "구매 희망 장비",
    region: "경북",
    year: 2021,
    price: null,
    priceNegotiable: true,
    status: "구매요청",
    condition: "중고",
    manufacturer: "제조사 무관",
    tradeOptions: ["가격 협의 가능", "직접 확인 가능"],
    description:
      "원형·각형 베일 작업이 가능한 중고 베일집게를 찾습니다. 유압 호스 포함 제품을 우선하며, 경북 인근은 직접 확인 가능합니다.",
    createdAt: "2026-07-13",
    confirmedAt: "2026-07-16",
  },
  {
    id: "12",
    sellerId: "seller-gg-equipment",
    title: "로더용 다목적 작업 버킷",
    category: "축산 작업기기",
    subcategory: "버킷",
    region: "경기",
    year: 2021,
    price: 7_800_000,
    priceNegotiable: true,
    status: "판매중",
    condition: "중고",
    manufacturer: "FarmTools",
    model: "FB-210",
    usageHours: 640,
    tradeOptions: ["가격 협의 가능", "시운전 가능"],
    description:
      "퇴비와 사료 운반에 사용한 로더용 다목적 버킷입니다. 용접부 보강과 도색을 완료했으며, 연결 규격은 현장 확인 가능합니다.",
    createdAt: "2026-07-14",
    confirmedAt: "2026-07-17",
  },
  {
    id: "13",
    sellerId: "buyer-jn-company",
    title: "전남 지역 3~5톤 중고 TMR 배합기 구합니다",
    category: "삽니다",
    subcategory: "구매 희망 장비",
    region: "전남",
    year: 2018,
    price: 45_000_000,
    priceNegotiable: true,
    status: "구매요청",
    condition: "중고",
    manufacturer: "제조사 무관",
    model: "3~5톤급",
    tradeOptions: ["가격 협의 가능", "직접 확인 가능", "운송 협의 필요"],
    description: "전남 농장에서 사용할 3~5톤급 TMR 배합기를 찾습니다. 스크류와 감속기 상태가 양호하고 시운전 가능한 장비를 우선합니다. 전남·전북 지역은 직접 확인할 수 있습니다.",
    createdAt: "2026-07-17",
    confirmedAt: "2026-07-18",
  },
  {
    id: "14",
    sellerId: "buyer-gn-farm",
    title: "사료 컨베이어 감속모터와 체인 부품 구합니다",
    category: "삽니다",
    subcategory: "부품 구함",
    region: "경남",
    year: 2020,
    price: null,
    priceNegotiable: true,
    status: "구매요청",
    condition: "중고",
    manufacturer: "제조사 확인 필요",
    model: "사진 확인 후 협의",
    tradeOptions: ["가격 협의 가능", "택배 가능", "규격 확인 필요"],
    description: "사료 이송 컨베이어에 사용할 감속모터와 체인 세트를 구합니다. 명판과 축 규격 사진을 먼저 보내주시면 호환 여부를 확인하겠습니다. 신품 재고와 상태 좋은 중고 모두 제안받습니다.",
    createdAt: "2026-07-18",
    confirmedAt: "2026-07-18",
  },
  {
    id: "15",
    sellerId: "buyer-jj-farm",
    title: "제주 운송 가능한 중고 사료공급기 찾습니다",
    category: "삽니다",
    subcategory: "구매 희망 장비",
    region: "제주",
    year: 2019,
    price: 12_000_000,
    priceNegotiable: true,
    status: "구매요청",
    condition: "중고",
    manufacturer: "제조사 무관",
    model: "500L 이상",
    tradeOptions: ["가격 협의 가능", "운송 협의 필요", "작동 영상 요청"],
    description: "500L 이상 사료공급기를 찾습니다. 제주 반입 운송이 가능하거나 항구까지 운송 협의 가능한 장비를 우선합니다. 현재 작동 영상과 배터리·엔진 상태를 함께 알려주세요.",
    createdAt: "2026-07-18",
    confirmedAt: "2026-07-18",
  },
];

export function getListingById(id: string): Listing | undefined {
  return listings.find((listing) => listing.id === id);
}
